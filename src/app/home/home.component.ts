...
export class HomeComponent {
  ...
  switchToLithuanian(): void {
    document.addEventListener('DOMContentLoaded', function () {
      document.querySelectorAll('[data-translate]').forEach((elem) => {
        if (elem.textContent) {
          let translation = elem.textContent;
          translation = TRANSLATIONS.filter(
            (phrase) => phrase.en === elem.textContent
          )[0]?.lt;
          if (translation) {
  ...
  changeLanguage(): void {
    window.location.href = this.language === 'lt' ? 'en' : '';
  }
}
